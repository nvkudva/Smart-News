...
import { searchPlacesAction, setPlacesAction } from '../lib/actions';
...
export function PlacePicker({ initial }: { initial: PickedPlace[] }) {
  ...
  const store = (list: PickedPlace[]) => {
    // The scorer substring-matches the names against the cluster's own free
    // text, so the first array carries bare names, never display labels.
