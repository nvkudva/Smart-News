 ...
 * a place name. A running story reports its latest movement and everything
 * else reports when it broke. The detail page carries both.
 */
export function storyWhen(s: { first_seen: number; last_seen: number }): string {
  return ago(running(s) ? s.last_seen : s.first_seen);
}
