...
The two are joined by `scripts/d1/hydrate.ts` (pull 5-day window down, delete and rebuild the local file) and `scripts/d1/sync.ts` (push what this cycle touched back up). `.github/workflows/cycle.yml` runs hydrate → cycle → sync every 15 minutes on a runner with no persistent disk, so D1 is the only durable copy of the working set.
...
- **The schema is written three times and has already drifted.** `src/lib/db.ts:20` is authoritative for local, `scripts/d1/sync.ts:18` re-declares it for D1, and `scripts/d1/hydrate.ts` hardcodes column lists a fourth time. The D1 copy has no `UNIQUE` on `articles.url`, no foreign keys, and no `articles_hash` index. Adding a column means editing four places, and forgetting one is silent.
- **`scripts/d1/sync.ts:63` `push()` builds `INSERT OR REPLACE` from a column list, so any column omitted from that list is nulled on every sync.** `sources` is pushed without `bias` (`scripts/d1/sync.ts:140`) while `scripts/d1/hydrate.ts:56` pulls it — the column can never hold a value.
...
- Duplication: `scripts/pipeline/run.ts` is `scripts/pipeline/cycle.ts` minus the ingest call and the `.t0` stamp.
...
- **`reap()` can permanently delete good rows.** `scripts/d1/sync.ts:91` deletes every D1 cluster absent from the freshly hydrated local file. The local file is scratch built by a paged `hydrate` (`scripts/d1/hydrate.ts:26`); if that pull is short for any reason the deletion is unrecoverable, and the only guard is a 10% heuristic (`scripts/d1/sync.ts:100`).
...
| P1 | Make one schema definition authoritative and generate the D1 one from it | `scripts/d1/sync.ts:18` | D1 lacks `articles.url UNIQUE`, the FKs, and `articles_hash`; four column lists must be edited in lockstep |
| P1 | Add `bias` to the `sources` push column list | `scripts/d1/sync.ts:140` | `INSERT OR REPLACE` nulls the column on every sync, so it can never hold data |
| P1 | Make `reap` refuse to delete unless hydrate reported a complete pull | `scripts/d1/sync.ts:100` | A short hydrate silently permanently deletes real clusters from the only durable store |
...
| P1 | Add a retention pass that prunes D1 rows outside the window | `scripts/d1/sync.ts:131` | Nothing ever deletes old articles or bodies; storage grows without limit |
...
| P2 | Drop `scripts/pipeline/run.ts` or make it call `cycle` with a flag | `scripts/pipeline/run.ts:8` | Duplicates `cycle.ts` minus ingest; two files drift on the same reporting logic |
